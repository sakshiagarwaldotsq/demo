...
wpcr3.submit = function(e) {
	...
	if (wpcr3.mousemove_total <= wpcr3.mousemove_need || wpcr3.keypress_total <= wpcr3.keypress_need) {
		...
		jQuery( ".wpcr3_required" ).addClass( "errreview" );
		return false;
	}
	...
	if (c2_fail) {
		...
		jQuery( ".wpcr3_fconfirm2" ).addClass( "reviewcheck" );
		return false;
	}
	...
	wpcr3.ajaxPost(parent, ajaxData, function(err, rtn) {
		...
		//alert('Thank you! Reviews need to be approved by admin before going live.');
		 var  homeurl = window.location.origin;
		$(window).attr('location', homeurl+'/thank-you/?msg=1');
		$(window).scrollTop(0);
		wpcr3.clearFields(); 
